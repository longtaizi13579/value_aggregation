...
for epoch in range(training_args.train_epoch):
    ...
    for idx, batch in enumerate(
        ...
        if (idx + 1) % 200 == 0:
            model_engine.save_checkpoint(f"{model_args.save_dir}-5-16-_epoch_{epoch}_step_{idx}_{exp_id}")
    ...
    # 每个 epoch 结束再存一份
    model_engine.save_checkpoint(f"5-16-{model_args.save_dir}_epoch_{epoch}_{exp_id}")
