...
from models import Value_Aggregation_Gather, VAPPT_Gather, InforNCE_and_Eigenvalue, InforNCE_and_Generative_Hops
    ...
    for every_file in tqdm(all_files, desc="Loading raw data"):
        ...
        with open(now_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not(len(a_dict["query"]) > 5 and len(a_dict["positive"]) > 10 and len(a_dict["negative"]) > 10):
                    # print(f"Warning: 跳过一个样本，query/positive/negative 不能为空 | {a_dict}")
                    continue
...
def SIGReg(x, global_step, num_slices=512):
    """
    ...
    - num_slices: 投影方向数量（对应论文中的|M|），默认512
    返回：
...
def evaluation(eval_dataloader, model_engine, step, output_dir="./gram_results"):
    ...
    # Q 和 N 拼在一起算 sigreg loss
    sigreg_loss = SIGReg(torch.cat([Q, N], dim=0), step)
    ...
    print(f"[Loss] sigreg loss           : {sigreg_loss:.6f}")
    ...
    with open(loss_stats_path, "a", encoding="utf-8") as f:
        ...
        f.write(f"Sigreg loss: {sigreg_loss:.10f}\n")
    ...
    cov_eigenvalues = torch.linalg.eigvalsh(cov_matrix)  # [D]，升序
...
# encode_ds.save_to_disk("~/autodl-tmp/va_scl_data")  # 如果不想落盘可以注释掉
...
# ffn_weight_lr = 1e-2    # model.ffn.weight
# base_lr = 3e-5    
...
for epoch in range(training_args.train_epoch):
    ...
    for idx, batch in enumerate(
        ...
        # all_embeddings = torch.cat([full_query_embedding, full_negative_embedding], dim=0)
        # norm_embeddings = normalize_embedding(all_embeddings)
        # r_loss = SIGReg(norm_embeddings, idx)
        final_loss = loss # + lamda * r_loss
        ...
        if (idx + 1) % 200 == 0:
            model_engine.save_checkpoint(f"{model_args.save_dir}-4-11-_epoch_{epoch}_step_{idx}_{exp_id}")
    ...
    # 每个 epoch 结束再存一份
    model_engine.save_checkpoint(f"4-11-{model_args.save_dir}_epoch_{epoch}_{exp_id}")
